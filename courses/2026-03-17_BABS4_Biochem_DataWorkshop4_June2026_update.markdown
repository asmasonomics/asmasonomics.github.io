---
layout: page
permalink: /courses/BABS4_Biochem_DataWorkshop4_March2026_JuneUpdate
---
...
Or, we may want to specify a list of names: <code>data=subset(withsymbols, symbol %in% c("muA", "muB", "gam"))</code>. Remember, your report is all about <b>narrative</b>.<br/><br/>
</p>
...
# install and load the circlize library
install.packages('circlize')
library(circlize)

# we will define a function which contains the entire circos plot render
# we then run the function having created a PDF for it to be saved into
# the circos plot is not rendered properly in the dev so can't just use ggsave()

# acknowledgment to a 2026 BABS4 GEBI student who really helped to get the 
# circos plot looking nice rather than super basic!

draw_circos <- function() {
  
  #acknowledgment to a 2026 BABS4 GEBI student who really helped to
  #get the circos plot looking nice rather than super basic!
  ...
  # initialise circos
  circos.initialize(factors = "L42023.1",
                    xlim = matrix(c(0, genome_len), ncol = 2))
  
  # genome length track
  circos.track(factors = "L42023.1", ylim = c(0, 1),
               track.height = 0.05, bg.border = NA,
               panel.fun = function(x, y) {
                 circos.axis(h = "top", major.at = seq(0, genome_len, 2e5),
                             labels = paste0(seq(0, 1.8, 0.2), " Mb"),
                             labels.cex = 0.6,
                             labels.facing = "clockwise",
                             major.tick.length = 0.3)
               })
  
  # draw prophage region
  # prophage region as a coloured arc
  circos.track(factors = "L42023.1", ylim = c(0, 1),
               track.height = 0.06, bg.border = NA,
               panel.fun = function(x, y) {
                 # Draw the prophage region as a black rectangle
                 circos.rect(1558774, 0.2, 1597183, 0.8,
                             col = "black", border = "black")
                 # Add a label above
                 circos.text(mean(c(1558774, 1597183)), 1.5,
                             "Mu prophage",
                             facing = "bending.inside",
                             niceFacing = TRUE, cex = 0.7)
               })
  
  # DEA heatmap
  # Map log2FC values to a red-white-blue colour ramp
  col_fun_dea <- colorRamp2(c(-3, 0, 3), c("blue", "white", "red"))
  
  circos.track(factors = "L42023.1", ylim = c(0, 1),
               track.height = 0.1, bg.border = "black",
               panel.fun = function(x, y) {
                 for (i in seq_len(nrow(dea))) {
                   if (!is.na(dea$log2FC[i])) {
                     circos.rect(dea$start[i], 0,
                                 dea$end[i],   1,
                                 col = col_fun_dea(dea$log2FC[i]),
                                 border = NA)
                   }
                 }
               })
  ...
  # GC content interior
  circos.track(factors = "L42023.1", ylim = c(min(gc$GC), max(gc$GC)),
               track.height = 0.18, bg.border = "black",
               panel.fun = function(x, y) {
                 circos.lines(gc$start, gc$GC, col = "black", lwd = 0.5)
                 # Add y-axis labels for GC scale
                 circos.yaxis(side = "left", at = round(seq(min(gc$GC),
                                                            max(gc$GC),
                                                            length.out = 3), 1),
                              sector.index = "L42023.1",
                              labels.cex = 0.35)
               })
  
  # plot centre title
  text(0, 0.05, "H. influenzae kw20", cex = 1.1, font = 2)
  text(0, -0.05, "(L42023.1, 1.83 Mb)", cex = 0.8)
  
}

# create pdf, run function and "drop" image into the file
pdf("plots/circos.pdf", width = 8, height = 8)
draw_circos()
dev.off()


```
![circos](/assets/coursefiles/2024-03_66I/plots/04_circos_002.png){:class="img-responsive"}
<p align="justify">
